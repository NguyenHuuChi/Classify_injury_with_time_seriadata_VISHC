...
class Read_data_runing:
    def __init__(self,file_path):
        ...
        self.Header1=read_specific_rows(self.file_path,3)
        self.Header2=read_specific_rows(self.file_path,4)
        # print(self.Header2)
...
path_03="TuanAnhRunning07.csv"
path3=Read_data_runing(path_03)


List_to_read=["Hip","Knee"]

# data_frame_model= path3.read_multiple_columns(List_to_read)
# print(data_frame_model)
...
data_run_right=pd.read_csv("data_run__right_leg_processed__.csv")
...
def read_cycle_left_right(start_frame,end_frame,check_left =True):
    information_cycle=[]
    phase_split_line=[]
...
def read_data(different_time_,list_file,List_column_to_read):
    ...
    for index,file_path in enumerate(list_file):
        ...
        print(start_frame_model,end_frame_model,"Start end frameeeeeeeeeeeeeeeeeeeeee")
        ...
        for key in data_frame_file.keys() :
            ...
            if key in All_data_frame : # {"column" : [[[model],[1,3],[3,4]],[split_line]]}
                ...
                if key[0]=='L' :
                    information_cycl, split_lin=read_cycle_left_right(start_frame=start_frame_model,end_frame=end_frame_model,check_left=True)
                    ...
                    information_cycl, split_lin=read_cycle_left_right(start_frame=start_frame_model,end_frame=end_frame_model,check_left=False)
                    flex     = data_frame_file[key][0]
                    ab       = data_frame_file[key][1]
                    rotation = data_frame_file[key][2]
                    for cycle in information_cycl:
...
def plot_average_with_error(interp_data_cycle,split_cycle,title1):
    ...
    title="Average with Standard Error of " + title1 +" in running motion"
...
different_time_from03=different_time[3:6]
# different_time_from03.append(different_time[7])
list_file=[
    "TuanAnhRunning03.csv",
    "TuanAnhRunning04.csv",
    "TuanAnhRunning05.csv",
    # "TuanAnhRunning06.csv","TuanAnhRunning07.csv"
    ]
# list_file=["TuanAnhRunning05.csv","TuanAnhRunning07.csv"]
List_to_read=["Hip"]
All_data= read_data(different_time_from03,list_file=list_file,List_column_to_read=List_to_read)
# print(All_data)
